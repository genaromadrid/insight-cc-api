...
var ERRORS = [
  "value undefined in pub_keys or reqSigs",
  "incorrect format pub_keys must be Array and reqSigs integer",
  "the amount the item in pub_keys must be less than equal to reqSigs",
  "verify any pub_keys is incorrect" 
]
...
AddressController.prototype._processingBulk = function(bulk){ 
 ...
 return new Promise((resolve,reject) => {
    bulk.forEach(function(item){ 
        ...
        indexError = ((item.reqSigs === undefined) || (item.pub_keys === undefined)) ? 0 : indexError;
        indexError = (!Number.isInteger(item.reqSigs) || !util.isArray(item.pub_keys)) ? 1 : indexError;
  
        if (indexError == undefined){
            indexError =  ((item.reqSigs <= 0) || (item.reqSigs > item.pub_keys.length)) ? 2 : indexError;
            if (indexError == undefined){
              let pub_keys = item.pub_keys.filter(function(pubKey){
                  return bitcore.PublicKey.isValid(pubKey);
              });
              indexError = (item.pub_keys.length !== pub_keys.length) ? 3 : indexError; 
          }
        }  
        if (indexError == undefined){
