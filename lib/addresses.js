...
AddressController.prototype.createAddresses = function(req, res){
  let multisign = req.body.multisign;
  let network = ((req.body.network == undefined)||(req.body.network !='litecoin')) ? bitcore.Networks.testnet : bitcore.Networks.mainnet;
...
AddressController.prototype._processingBulk = function(bulk){ 
 ...
 return new Promise((resolve,reject) => {
    bulk.forEach(function(item){ 
        ...
        indexError = ((item.reqSigs === undefined) || (item.pubKeys === undefined)) ? 0 : indexError;
        indexError = (!Number.isInteger(item.reqSigs) || !util.isArray(item.pubKeys)) ? 1 : indexError;
  
        if (indexError == undefined){
            indexError =  ((item.reqSigs <= 0) || (item.reqSigs > item.pubKeys.length)) ? 2 : indexError;
            if (indexError == undefined){
              let pubKeys = item.pubKeys.filter(function(pubKey){
                  return bitcore.PublicKey.isValid(pubKey);
              });
              indexError = (item.pubKeys.length !== pubKeys.length) ? 3 : indexError; 
          }
        }  
        if (indexError == undefined){
          multisignContainer.multisign_valid.push(item);
        }else{  
          let invalidItem = JSON.parse(JSON.stringify(item));
          invalidItem.error = ERRORS[indexError]; 
          multisignContainer.multisign_not_valid.push(invalidItem);
        }
    });
     multisignContainer.multisign_valid = multisignContainer.multisign_valid.sort((a, b) => (a.index > b.index));
    resolve(multisignContainer);
 }).catch(function(err){
   throw err;
 }); 
}
   this._populateMulsignValid(multisign,network).then(function(p2sh_addresses){
    let addresses=p2sh_addresses.map(function(multisign){
        let address=bitcore.Address(multisign.pub_keys,multisign.reqSigs,network);
        return address.toString();
    }) 
    res.jsonp(addresses);
   }).catch(function(e){
     self.common.handleErrors(e);
   }); 
};

AddressController.prototype._populateMulsignValid = function(multisign){ 
 return new Promise((resolve,reject) => {
   let p2sh_addresses = multisign.filter(function(multisign){ 
   let isValid = ((multisign.reqSigs !== undefined) && ((multisign.pub_keys !== undefined) && (util.isArray(multisign.pub_keys))));
   isValid = isValid && ((Number.isInteger(multisign.reqSigs) && (multisign.reqSigs > 0)) && (multisign.reqSigs <= multisign.pub_keys.length));
   if (isValid){
          let pub_keys=multisign.pub_keys.filter(function(pubKey){
          return bitcore.PublicKey.isValid(pubKey);
        });
        isValid=(multisign.pub_keys.length === pub_keys.length);
    } 
     return (isValid);
    }) 
    resolve(p2sh_addresses);
 }).catch(function(err){
   throw err;
 }); 
}
