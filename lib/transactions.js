...
TxController.prototype.checkAndStoreCCTransaction = function (txid, callback) {
  ...
  this.getTransformTransaction(txid, function (err, rootTransformedTx) {
    ...
    var transformedTx = undefined;
    this._evaluateTransformTransaction(rootTransformedTx).then((results) => {
      if (results.isIssueanceTx === true) {
        console.log('original ' + JSON.stringify(rootTransformedTx));
        console.log('previous ' + JSON.stringify(results.transformedTx));
        rootTransformedTx.assets[0].assetId = results.transformedTx.assets[0].assetId;
        console.log('new ' + JSON.stringify(rootTransformedTx));
...
TxController.prototype._evaluateTransformTransaction = function (transformedTx) {
  ...
  return new Promise((resolve, reject) => {
    if ((transformedTx.assets == undefined) || (transformedTx.assets.length == 0)) {
      ...
      self._searchIssuanceTx(transformedTx)
        .then(self._evaluateTransformTransaction)
        .then((container) => {
          results.transformedTx = container.transformedTx;
          results.isIssueanceTx = true;
          resolve(results);
        })
    }
  });
...
TxController.prototype._searchIssuanceTx = function (transformedTx) {
  ...
  return new Promise((resolve, reject) => {
    lodash.forEach(transformedTx.vin, function (input) {
      console.log('itera inputs');
      self._isIssueanceTx(input).then((results) => {
        if (results.isIssueanceTx === true) {
          this.getTransformTransaction(results.issueanceTx.hash, function (err, transformedTx) {
            if (err) {
              reject(err);
            }
...
TxController.prototype._isIssueanceTx = function (input) {
  var self = this;
  return new Promise((resolve, reject) => {
    self.node.getDetailedTransaction(input.txid, function (err, input_tx) {
      ...
      var isIssueanceTx = lodash.some(input_tx.outputs, function (out) {
        return out.scriptAsm.includes('OP_RETURN ');
      });
      var results = {
        isIssueanceTx: isIssueanceTx,
