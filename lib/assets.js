...
<<<<<<< HEAD
=======
const ESTIMATE_FEE_URL = config.ESTIMATE_FEE_URL;
>>>>>>> Applied some fixes for transfer asset
...
AssetController.prototype.buildSendTransaction = function(req, res) {
  const body = JSON.parse(JSON.stringify(req.body));
  body.financeOutputTxid = Buffer.from(body.financeOutputTxid, 'hex')
    ...
    this._getUtxo(address, this.addressController.ccBuilderTransformUtxo)
      .then((utxos) => {
       ...
       if (handle.type === 'send'){
           let container = {
             addresses:[address],
             assets:[body.to[0].assetId],
             utxos:utxos
          }
           return this._getUtxosFrom(container);
         }else{
